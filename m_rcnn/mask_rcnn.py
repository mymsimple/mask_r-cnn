...
class MaskRCNN(object):
    ...
    def build(self):
        ...
        if self.train_flag:
            ...
            mrcnn_mask = common.build_fpn_mask_graph(rois,
                                                     mrcnn_feature_maps,
                                                     input_image_meta,
                                                     mask_pool_size,
                                                     self.class_num,
            ...
            # Model
            # batch_images, batch_class_ids, batch_boxes
            inputs = [input_image,
                      input_image_meta,
                      input_rpn_match,
                      input_rpn_bbox,
    ...
    def detect(self, images_info_list, verbose=0):
        ...
        if verbose:
            pass
        # TODO 模型输入输出 3个输入 7个输出
        # Run object detection

        detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, _, _, _ = \
    ...
    def filter_box(self, final_rois, final_class_ids, final_scores, final_masks):
        ...
        for i in range(0, length):
            for j in range(i + 1, length):
                iou1 = self.calculate_IoU(final_rois[i], final_rois[j])
                iou2 = self.calculate_IoU(final_rois[j], final_rois[i])
    ...
    def un_mold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        low_confidence = np.argwhere(scores < 0.50)
